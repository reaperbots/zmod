...
if len(USER_SESSION_STRING) != 0:
    try:
        user = TgClient(
            ...
            no_updates=True,
            app_version="@Reaperzclub Session",
            device_model="@Reaperzclub Bot",
            system_version="@Reaperzclub Server",
        ).start()
...
OS_PORT = environ.get("PORT")
if BASE_URL:
    Popen(
        f"gunicorn web.wserver:app --bind 0.0.0.0:{OS_PORT} --worker-class gevent --log-level error",
        shell=True,
    )
...
trackers = check_output(
    "curl -Ns https://ngosang.github.io/trackerslist/trackers_all_http.txt | awk '$0' | tr '\n\n' ','",
    shell=True,
).decode("utf-8").rstrip(",")
...
with open(
    ...
    if TORRENT_TIMEOUT is not None:
        a2c.write(f"bt-stop-timeout={TORRENT_TIMEOUT}\n")
    a2c.write(f"bt-tracker=[{trackers}]")
hrun([
    pkg_info["pkgs"][0],
